...
(function(joo_global_object)
   {"use strict";
    ...
    var
     ...
     test_server_addr=caml_new_string(">_test_server_addr_<"),
     ...
     msg=caml_new_string("Hi there, Server! I am test client!!"),
     opt=[0,1];
    ...
    var
     ...
     _cq_=caml_new_string("fail: cannot connect to remote_socket "),
     _co_=caml_new_string("fail: start_comm_layer with server_url="),
     _cJ_=caml_new_string("ERROR: OMQ socket already closed!"),
     _cI_=[0,0],
     _cE_=caml_new_string("Called send before any recv on a REP socket"),
     _cF_=caml_new_string("Called send twice in a row on a REP socket"),
     _cG_=caml_new_string("Called send twice in a row on a REQ socket"),
     _cH_=
      caml_new_string
       ("Must provide OMQ Socket Identity when sending on Router socket"),
     _cD_=caml_new_string("No connected services to send msg to!!"),
     _cC_=
      [0,
       [11,
     ...
     _cO_=caml_new_string("INFO: Context is terminating correctly!\n"),
     _cL_=[0,[2,0,[12,10,0]],caml_new_string("%s\n")],
     _cK_=caml_new_string("ERROR: Operation on closed context"),
     _cX_=
      [0,
       [11,
        caml_new_string(">>>TEST: Sucessfully received message ("),
        [2,0,[11,caml_new_string(") from SERVER!\n"),0]]],
       caml_new_string
        (">>>TEST: Sucessfully received message (%s) from SERVER!\n")],
     _cY_=
      [0,
     ...
     _cV_=
      [0,
       [11,caml_new_string(">>>TEST: Sucessfully sent request to server\n"),0],
       caml_new_string(">>>TEST: Sucessfully sent request to server\n")],
     _cW_=
      [0,
       [11,caml_new_string(">>>TEST: About to wait for reply\n"),0],
       caml_new_string(">>>TEST: About to wait for reply\n")],
     _cT_=
      [0,
       [11,
        caml_new_string
         (">>>TEST: Sucessfully connected to OMQ Test Server listening on rand local ("),
        [2,0,[11,caml_new_string(")\n"),0]]],
       caml_new_string
        (">>>TEST: Sucessfully connected to OMQ Test Server listening on rand local (%s)\n")],
     _cU_=
      [0,
       [11,caml_new_string(">>>TEST: About to send request to server\n"),0],
       caml_new_string(">>>TEST: About to send request to server\n")],
     _cR_=
      [0,
       [11,
        caml_new_string
     ...
     _cS_=
      [0,
       [11,
        caml_new_string
         (">>>TEST: Trying to create a REQ socket and connect to OMQ TEST SERVER\n"),
        0],
       caml_new_string
        (">>>TEST: Trying to create a REQ socket and connect to OMQ TEST SERVER\n")],
     _cP_=
      caml_new_string
       (">>>TEST: I am the OMQ CLIENT, trying to start the PCL\n");
       ...
       {var
        ...
        return make_with_timeout_blocking_promise
                (sckt,
                 ...
                 unblocked_state,
                 timeout_during)}
      return fail([0,OMQ_Exception,_cD_])}
    function get_identity(sckt)
     {ensure_not_blocked_and_open(sckt);return sckt[4]}
    function internal_close(sckt)
     {if(sckt[2]){sckt[2] = 0;return 0}throw [0,OMQ_Exception,_cJ_]}
    var exists_singleton_context=[0,0];
    function ensure_is_open(ctx)
     {var _dh_=1 - ctx[2];if(_dh_)throw [0,OMQ_Exception,_cK_];return _dh_}
    print_string(_cP_);
    if(exists_singleton_context[1])
     {caml_call1(printf(_cL_),err_msg);throw [0,OMQ_Exception,err_msg]}
    ...
    function _cM_(my_id){return return$0([0,my_id,[0,0,1]])}
    var
     ppx_lwt_0=
      backtrace_bind
       (function(exn)
    ...
    function _cQ_(param)
     {var ctx=param[2],unique_id=param[1];
      caml_call1(printf(_cR_),unique_id);
      printf(_cS_);
      var
       _c3_=create$0(0),
       _c4_=OmqSocketIdMap[1],
       _c5_=RemoteSocketMap[1],
      ...
      var _c1_=[0,_a_(_cq_,test_server_addr)];
      function fail_callback$0(_df_)
       {return make_exn_fail_callback(_c1_,resolver,_df_)}
      var address=test_server_addr.toString();
      ...
      function _c2_(local)
       {var _c$_=sckt[4];
        caml_call3(printf(_cC_),_c$_,test_server_addr,local);
        sckt[5] = add(local,sckt[5]);
       ...
       ppx_lwt_0=
        backtrace_bind
         (function(exn)
          ...
          promise,
          _c2_);
      function _c6_(local$0)
       {caml_call1(printf(_cT_),local$0);
        printf(_cU_);
        if(opt)var sth=opt[1],block=sth;else var block=1;
        ensure_not_blocked_and_open(sckt);
        var
        ...
        switch(sckt[1])
         {case 0:
           ...
           if(typeof match === "number")
            var ppx_lwt_0=fail([0,OMQ_Exception,_cE_]);
           else
            if(0 === match[0])
          ...
          case 1:
           ...
           if(typeof match$1 === "number")
            ...
            if(0 === match$1[0])
             var ppx_lwt_0=fail([0,OMQ_Exception,_cG_]),switch$0=1;
            else
             var switch$0=0;
           if(! switch$0)
          ...
          default:var ppx_lwt_0=fail([0,OMQ_Exception,_cH_])}
        function _c7_(param)
         {printf(_cV_);
          printf(_cW_);
          ensure_not_blocked_and_open(sckt);
          if(is_empty(sckt[12]))
           ...
           {var
            ...
            sckt[7]
            =
            caml_call3(OmqSocketIdMap[4],id,[0,local,remote],sckt[7]);
            var ppx_lwt_0=return$0([0,id,msg])}
          function _c8_(param){var msg=param[2];return return$0(msg)}
          var
           ppx_lwt_0$0=
            backtrace_bind
             (function(exn)
          ...
          function _c9_(msg)
           {var str_msg=msg[1];
            caml_call1(printf(_cX_),str_msg);
            printf(_cY_);
            ensure_is_open(ctx);
            var accu=0,param=ctx[1];
            for(;;)
             {if(param)
               {var l=param[2],x=param[1],_c__=get_identity(x);
                if(caml_string_notequal(get_identity(sckt),_c__))
